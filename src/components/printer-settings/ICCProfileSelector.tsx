...
export const ICCProfileSelector: React.FC<ICCProfileSelectorProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Upload Header */}
      <div className="flex items-center justify-between gap-3 p-3.5 bg-[#0a0b10] border border-slate-800 rounded-2xl">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-indigo-500/20 text-indigo-300 rounded-xl border border-indigo-500/30">
            <HardDrive className="w-5 h-5" />
          </div>
          <div>
            <h5 className="text-xs font-bold text-white">Importar Perfil ICC Customizado (.icc / .icm)</h5>
            ...
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="font-bold text-xs text-white">{icc.name}</span>
                  <span className="text-[9px] font-mono font-bold px-1.5 py-0.5 bg-slate-800 text-purple-300 rounded border border-slate-700">
                    {icc.colorSpace}
                  </span>
                  {icc.isCustom && (
                    <span className="text-[9px] font-bold px-1.5 py-0.5 bg-amber-500/20 text-amber-300 rounded border border-amber-500/30">
                      Customizado
                    </span>
                  )}
                </div>
                <p className="text-[11px] text-gray-400">{icc.description}</p>
                <span className="text-[10px] text-gray-500 font-mono block">Arquivo: {icc.filename}</span>
              </div>
