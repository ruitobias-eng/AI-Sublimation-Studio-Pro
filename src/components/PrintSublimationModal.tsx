...
export const PrintSublimationModal: React.FC<PrintSublimationModalProps> = ({
  ...
  const costEstimation = calculateInkAndCost(
    currentProduct?.widthMm || 204,
    currentProduct?.heightMm || 90,
    65
  );
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar touch-scroll-y min-h-0 p-4 sm:p-6 space-y-6">
              ...
              <div className="lg:col-span-7 flex flex-col space-y-3">
                <div className={`relative border rounded-2xl p-4 flex flex-col items-center justify-center min-h-[340px] overflow-hidden ${
                  ...
                  <div
                    ...
                    style={{
                      width: `${Math.min(360, (currentProduct?.widthMm || 204) * 1.5) * (zoomLevel / 100)}px`,
                      height: `${Math.min(220, (currentProduct?.heightMm || 90) * 1.5) * (zoomLevel / 100)}px`,
                  ...
                  {showCropMarks && (
                    <>
                      ...
                      <div className="absolute top-2 right-2 text-[9px] font-mono font-bold text-slate-400">┐ {currentProduct?.widthMm || 204}mm</div>
                      <div className="absolute bottom-2 left-2 text-[9px] font-mono font-bold text-slate-400">└ {currentProduct?.heightMm || 90}mm</div>
          ...
          {/* TAB 5: Suporte Técnico & Dicas de Sublimação */}
          {activeTab === 'support' && (
            <div className="space-y-6">
              {/* Custom Presets Form */}
